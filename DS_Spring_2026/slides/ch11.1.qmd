...
- Grading of MP2 was concluded and published.
- PS4 grading is pending due to a mistake in publishing a wrong `.txt` starter file.
    - Submission on GitHub encountered hitch due to the modifications made to correct the mistake.
    - Consequently, Problem 1 of PS4 will not be graded.
    - Only Problem 2: Array-based Queue Implementation will be graded.
    - If you have not done so, please private message me your `arrayqueue.py` on Discord.
...
## Continuation on Binary Trees Application  
- Todays Lecture:
...
### Tasks 1:
- Add the methods `height` and `isBalanced` to the `LinkedBST` class. The `height` method returns the height of the tree, as defined in this chapter. 
    - The `isBalanced` method returns `True` if the height of the tree is less than twice the $log_2$ of its number of nodes, or `False` otherwise.
- Ensure the following lines of code in the testbst.py are uncommented before running the program.
   -  `# print("\nHeight =", tree.height())`
