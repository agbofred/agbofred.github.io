...
1. List the four general categories of collections defined in the learning objectives for Week 1.

2. Define the space/time trade-off in algorithm design.

3. If Insertion Sort were modified to use binary search to find the insertion point, why would the overall time complexity remain $O(n^2)$?
...
6. Explain what Vectorization is in the context of NumPy and why it provides massive speed gains over standard Python lists for numerical operations.

7. In a singly linked structure, what are the running time complexities for Access by position and Insertion at the beginning?

8. What is the primary purpose of introducing a dummy header node in a circular linked structure?

9. When comparing the memory usage of a full ArrayBag versus a LinkedBag of the same logical size, which implementation uses less memory, and what is the approximate memory cost difference in the worst case?
