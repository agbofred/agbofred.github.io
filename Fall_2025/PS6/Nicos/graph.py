...
class LinkedDirectedGraph(AbstractCollection):
    ...
    def spanTree(self, startLabel):
        tree = LinkedDirectedGraph()
        if startLabel not in self.vertices:
            return tree
        ...
        tree.addVertex(startLabel)

        while queue:
            current = queue.popleft()
            currentVertex = self.getVertex(current)


            for neighbor in currentVertex.neighboringVertices():
                nlabel = neighbor.getLabel()

                if nlabel not in visited:
                    visited.add(nlabel)

                    tree.addVertex(nlabel)
                    edge = currentVertex.getEdgeTo(neighbor)
                    tree.addEdge(current, nlabel, edge.weight)
                    queue.append(nlabel)
        return tree
    
    def shortestPaths(self, startLabel):

        dist = {label: float("inf") for label in self.vertices}
        dist[startLabel] = 0
        visited = set()
        ...
        while len(visited) < len(self.vertices):
            ...
            for v in self.vertices:
                if v not in visited and dist[v] < currentDist:
                    current = v
                    currentDist = dist[v]
            ...
            visited.add(current)
            vertexObj = self.getVertex(current)

            for neighbor in vertexObj.neighboringVertices():
                nlabel = neighbor.getLabel()
                edge = vertexObj.getEdgeTo(neighbor)
                newDist = dist[current] + edge.weight
                if newDist < dist[nlabel]:
                    dist[nlabel] = newDist
