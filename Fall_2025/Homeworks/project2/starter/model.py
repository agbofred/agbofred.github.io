""" This model defines the PFEvaluator Model and PFEvaluator, """
...
class PFEvaluator(object):
    ...
    def evaluate(self):
        while self.scanner.hasNext():
            ...
            if currentToken.getType() == Token.INT:
                ...
                if len(self.operandStack.pop) < 2:
                    raise AttributeError("Too few opreands on the stack")
    ...
    def computeValue(self, op, value1, valus2):
        result = 0
        theType = op.getType()
        if theType == Token.PLUS:
            result = value1 + valus2
