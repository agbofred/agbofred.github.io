...
31. What is the primary role of a stack in a backtracking algorithm? 

    A. To store the final solution 

    B. To remember alternative states or junk-tures to return to if a path fails

    C. To sort the possible moves in ascending order 

    D. To prevent the algorithm from visiting the same state twice 

32. What is an "activation record" (also called a stack frame)? 

    A. A list of all global variables in a program 

    B. A record of how many times a specific function has been called 

    C. The compiled bytecode of a single Python method 

    D. A data structure pushed onto the call stack containing a subroutine's data like parameters and return addresses

33. In the ArrayStack implementation, which index represents the "top" of the stack? 
    ...
    D. A random index chosen at instantiation 

34. What are the two specific fields required for the Node class in a linked stack implementation? 
...
35. When pushing an item onto a linked stack, what is the first step of the internal process? 

    A. Set a newNode to Node(data, self.items)

    B. Increment the self.size variable 

    C. Shift all existing items one position to the right 

    D. Clear the existing self.items pointer
...
2. List three fundamental operations supported on *all* collection types.
...
<!-- Newly Added Stack Portion  -->
11. During the infix-to-postfix conversion process, what types of items are stored on the stack?
    
   
12. When converting infix to postfix, what happens to the left parenthesis once its matching right parenthesis is processed?
    
  
13. What is the primary internal step required to pop an item from a linked stack?
    
    **Answer:** Set self.items to self.items.next.
14. What does the AbstractStack constructor initialize when a new stack is created?
    
   
15. Which two methods does the Scanner class provide to iterate through tokens in an arithmetic expression?
