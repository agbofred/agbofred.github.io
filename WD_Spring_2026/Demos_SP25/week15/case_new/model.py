...
class GraphDemoModel(object):
    ...
    def createGraph(self, rep, startLabel):
        ...
        for edge in edgeList:
            if not '>' in edge:
                ...
                if self.graph.containsEdge(fromLabel,
                    toLabel):
                    self.graph = None
                    return "Duplicate edge"
                self.graph.addEdge(fromLabel, toLabel,
                weight)
    ...
    def getGraph(self):
        ...
        if not self.graph:
            ...
            return str(self.graph) ## There was typo here before str(self._graph)
        
    # def getStartLabel(self):        # missing in the book model codes
    #     return self.startLabel  
    
    def run(self, algorithm):
        """Runs the given algorithm on the graph and
        returns its re0073ult, or None if the graph is
        unavailable."""
