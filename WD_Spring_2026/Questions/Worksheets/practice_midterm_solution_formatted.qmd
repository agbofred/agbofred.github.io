...
author: "Practice Midterm Solutions"
date: today
format:
  html:
    theme:
...
- ✅ Optimized for MySQL/MariaDB environments (phpMyAdmin)
- ✅ Industry-standard best practices for performance
- ✅ Referential integrity considerations
- ✅ Real-world database scenarios
- ✅ **Dataset:** 148 photos, 448 ratings, 10 normalized tables
:::
...
::: {.callout-important}
## 🔑 Foreign Key (FK) to Primary Key (PK) Mappings

As a developer, understanding these mappings is essential for accurate `JOIN` operations:

- **Continents & Countries:** `continents.ContinentCode` (PK) → `countries.ContinentCode` (FK)
- **Countries & Cities:** `countries.CountryCode` (PK) → `cities.CountryCode` (FK)
- **Users & Content:** `users.UserID` (PK) → `imagedetails.UserID`, `imagerating.UserID`, `posts.UserID` (FK)
- **Media Mapping:** `imagedetails.ImageID` (PK) → `imagerating.ImageID`, `postimages.ImageID` (FK)
- **Geography & Media:** `countries.CountryCode` (PK) → `imagedetails.CountryCode` (FK)
:::
...
::: {.callout-warning}
## 💡 Curriculum Insight (Foreign Keys)
